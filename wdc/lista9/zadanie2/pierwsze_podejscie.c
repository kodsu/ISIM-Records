...
void self_realokacja(struct list* origin, int l, int r){ 
    if(r == -1) 
        return;  
    struct node* l_wsk; struct node* r_wsk; 
    r_wsk = element(origin, r, 1); 
    if(r > 0){ 
        if(r_wsk == (origin->end)->prev) 
            return; 
    }
    l_wsk = element(origin, l, 1); 
    (l_wsk->prev)->next = r_wsk->next; 
    (r_wsk->next)->prev = l_wsk->prev; 
    struct node* last = origin->end->prev; 
    last->next = l_wsk; 
    l_wsk->prev = last; 
    r_wsk->next = origin->end; 
    origin->end->prev = r_wsk; 
    return; 
...
moj_typ main(){    
    ...
    for(int i = 0; i < Q; i++){  
        ...
        if(type == 'R'){ 
            ...
            if(from == to) 
                self_realokacja(&magazyn[from], l, r); 
        }
    }
    //remove_all(); 
}   