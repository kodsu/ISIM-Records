(*plik do ogarniecia i porozpisywania sobie rzeczy aby zrozumiec rzeczy ktore pojawily sie na wykladzie*)
let rec insert n xs =
  match xs with
  | [] -> [n]
  | x :: xs' ->
...
(*wszystkie zdefiniowane tutaj drzewa sa binarne*)
let rec map_tree f t =   (*mapuje (zamienia) kazdy element x (wartosc w wierzcholku) drzewa t na f(x), tzn zwraca drzewo po takiej zamianie*)
  match t with
  | Leaf -> Leaf
  | Node (l, v, r) -> Node (map_tree f l, f v, map_tree f r)

let rec filter_tree p t =  (*wybiera poddrzewo dfsowe drzewa o wierzcholkach, ktorych wartosci spelniaja p*)
  match t with
  | Leaf -> Leaf
  | Node (l, v, r) ->
      if p v 
...
let flatten t = fold_tree (fun l v r -> l @ [v] @ r) [] t  (* hehe beka, euler tour*)

let example_tree = Node (Node (Leaf, 1, Leaf), 2, Node(Leaf, 3, Leaf))

let rec find_bst x t = (*a to co niby jest? - funkcja ktora madrze przechodzi po'
    bst i sprawdza czy zawiera ono wierzcholek o wartosci x*)
  match t with
  | Leaf -> false
  | Node (l, v, r) -> 
      if x = v then true
