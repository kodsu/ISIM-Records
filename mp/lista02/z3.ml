...
let rec matrix_expt_fast (m : matrix) k = 
    match k with 
      | 0 -> matrix_id 
      | k -> match (k mod 2) with 
              | 0 -> let res = matrix_expt_fast m (k/2) in matrix_mult res res 
              | 1 -> let res = matrix_expt_fast m (k/2) in 
                matrix_mult(matrix_mult res res) m 
;;
...
let fib_fast k = 
    m_nd (matrix_expt_fast fib_base k);;
...
print_int (fib_fast 4);; print_newline();; 
print_int (fib_fast 100);; print_newline();; 
